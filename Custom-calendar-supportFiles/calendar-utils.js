...
export function getResourceDayView(_a){
    var _b = _a.events, 
    events = _b === void 0 ? [] : _b, 
    viewDate = _a.viewDate, 
    hourSegments = _a.hourSegments, 
    ...
    resourceView = _a.resourceView,
    resources = _a.resources,
    resourceWidth = _a.resourceWidth;
    if (!events) {
        events = [];
    }
    var dayViewEvents = [];
    var resourceDayViewEvents = [];
    if(resourceView && resources){
        events = [];
        resources.forEach(function(resource,resourceIndex){
            if(!resource.events){
                resource.events = [];
            }
            var startOfView = setMinutes(setHours(startOfDay(viewDate), dayStart.hour), dayStart.minute);
            ...
            var previousDayEvents = [];
            var eventsInPeriod = getEventsInPeriod({
                events: resource.events.filter(function (event) { return !event.allDay; }),
                periodStart: startOfView,
                periodEnd: endOfView
            });
            var sortedDayViewEvents = eventsInPeriod.sort(function (eventA, eventB) {
                return eventA.start.valueOf() - eventB.start.valueOf();
            });
            dayViewEvents = eventsInPeriod
                ...
                var eventWidth = 78;
                if(resourceWidth){
                   eventWidth = resourceWidth; 
                }else{
                    if(resources){
                        eventWidth = parseFloat(parseFloat(eventWidth/resources.length).toFixed(2));
                    }
                }
                var eventStart = event.start;
                var eventEnd = event.end || eventStart;
                ...
                var bottom = top + height;
                var overlappingEvents = sortedDayViewEvents.filter(function(otherEvents){
                    if((event.end > otherEvents.start && event.start < otherEvents.end) || (event.start == otherEvents.start) || (event.end == otherEvents.end)){
                        return true;
                    }
                });
                ...
                while (overlappingPreviousEvents.some(function (previousEvent,index) { 
                    if(previousEvent.exactLeft === currentEventLeft){
                        prevEventLeft = previousEvent.exactLeft;
                        previousEvent.width = parseFloat(parseFloat(eventWidth).toFixed(2));
                        return true;
                    }else{
                        prevEventLeft = null;
                        return false;
                ...
                top = parseFloat(parseFloat(top).toFixed(2));
                var style =""
                if(resourceWidth){
                    style = {'width':eventWidth+"px",'margin-left':currentEventLeft+"px"};
                    
                }else{
                    style = {'width':eventWidth+"%",'margin-left':currentEventLeft+"%"};
                ...
                var dayEvent = {
                    ...
                    endsAfterDay: endsAfterDay,
                    style:style,
                    resourceOffset:resourceIndex+1
                };
                if (height > 0) {
                    dayEvent.exactLeft = currentEventLeft;
                    previousDayEvents.push(dayEvent);
                }
                return dayEvent;
            })
            ...
            resourceObj = Object.assign(resourceObj,resource);
            resourceObj.events = dayViewEvents;
            resourceDayViewEvents.push(resourceObj);
        });
    }
    return resourceDayViewEvents;
}
...
export function checkResource(args){
    var draggedInPixels = args.draggedInPixels.x;
    var dayEvent = args.event;
    var eventWidth  = dayEvent.width;
    var resourceWidth = 200;
    var leftMargin = (dayEvent.left * 100) / resourceWidth;
    draggedInPixels += leftMargin
    var resourceId = dayEvent.resourceId;
    var resources = args.resources;
    if(parseInt(draggedInPixels.toString().replace("-","")) >= resourceWidth){
        var resourceIndex = parseInt(draggedInPixels / resourceWidth);
        resourceIndex = (parseInt(dayEvent.resourceOffset + resourceIndex) - 1);
        if(resources[resourceIndex]){
            return resources[resourceIndex].id;
        }else{
            return resourceId;     
        }
    }else{
